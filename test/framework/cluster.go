...
// NewReusableClusterE2ETest runs on a cluster used for multiple tests.
func NewReusableClusterE2ETest(t *testing.T, logger Logger, provider Provider, opts ...ClusterE2ETestOpt) *ClusterE2ETest {
	e := &ClusterE2ETest{
		T:                     t,
		Logger:                logger,
		Provider:              provider,
...
func (e *ClusterE2ETest) UninstallCuratedPackage(packagePrefix string, opts ...string) {
	...
	e.RunEKSA([]string{
		"delete", "package", "--cluster", e.ClusterName, packagePrefix, "-v=9",
		strings.Join(opts, " "),
	})
}
